...
class KnowledgeSource(AutoCodeMixin, SingleInstanceModel):
    CODE_PREFIX = 'KS'
    ...
    code = models.CharField('código', max_length=100, blank=True)
...
class KnowledgeDocument(SingleInstanceModel):
    ...
    source = models.ForeignKey(
        KnowledgeSource, on_delete=models.PROTECT, related_name='documents', verbose_name='fonte'
    )
    title = models.CharField('título', max_length=255)
...
class KnowledgeChunk(SingleInstanceModel):
    source = models.ForeignKey(
        KnowledgeSource, on_delete=models.PROTECT, related_name='chunks', verbose_name='fonte'
    )
    document = models.ForeignKey(
        KnowledgeDocument, on_delete=models.CASCADE, related_name='chunks', verbose_name='documento'
    )
...
class RAGChatMessage(SingleInstanceModel):
    ...
    session = models.ForeignKey(
        RAGChatSession, on_delete=models.CASCADE, related_name='messages', verbose_name='sessão'
    )
...
class RAGCitation(SingleInstanceModel):
    message = models.ForeignKey(
        RAGChatMessage, on_delete=models.CASCADE, related_name='citations', verbose_name='mensagem'
    )
    source = models.ForeignKey(
        KnowledgeSource, on_delete=models.PROTECT, related_name='citations', verbose_name='fonte'
    )
    document = models.ForeignKey(
        KnowledgeDocument,
        on_delete=models.PROTECT,
    ...
    def clean(self):
        super().clean()
        errors = {}
        for field_name in ('message', 'source', 'document', 'chunk'):
            pass
